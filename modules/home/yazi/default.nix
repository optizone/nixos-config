{ inputs, pkgs,  ... }:
{
  ...
  programs.yazi = {
    ...
    settings = {
      ...
      opener = {
        edit    = [ { run = "\${EDITOR:-vi} \"$@\"";          desc = "$EDITOR"; block = true; "for" = "linux"; } ];
        open    = [ { run = "xdg-open \"$1\"";                desc = "Open";                  "for" = "linux"; } ];
        reveal  = [ { run = "xdg-open \"$(dirname \"$1\")\""; desc = "Reveal";                "for" = "linux"; } ];
        extract = [ { run = "ya pub extract --list \"$@\"";   desc = "Extract here";          "for" = "linux"; } ];
        play    = [ { run = "mpv --force-window \"$@\"";      orphan = true;                  "for" = "linux"; } ];
      };
    ...
    keymap = {
      ...
      manager.prepend_keymap = [
        ...
        # X11
        # { on = "y"; run [ 'shell -- echo "$@" | xclip -i -selection clipboard -t text/uri-list', "yank" ] }
        ...
        { on = "!"; run = "shell fish --block"; desc = "Open $SHELL here"; for = "linux"; }

        # Mount plugin
        { on = "M"; run = "plugin mount"; desc = "Open mount menu"; }

        # Drag and drop via dragon
        { on  = "<C-n>"; run = "shell -- dragon -x -i -T \"$1\""; for = "linux"; }

        # cd back to the root of the current Git repository
        { on = [ "g" "r" ]; run = "shell -- ya emit cd \"$(git rev-parse --show-toplevel)\""; for = "linux"; }
      ];
    };
